 ...
 * @LastEditors: aoi
 * @LastEditTime: 2024-08-26 20:26:49
 * @Description: 
...
IOLcm::IOLcm(std::string robot_name): _running(true), _lcm("udpm://239.255.76.67:7667?ttl=1")
{
...
{
    int defaultPositions[19] = {90192, 5292, -44683, 199904, 100000, -100000, -90193, -5293, 44682, -199905, -100000, 100000, 0, 0, 0, 0, 0, 0, 0};
    for (int i = 0; i < 12; i++)
    {
        _lcm_Pub_Des.qDes[i] = defaultPositions[i] + 100.0;
        _lcm_Pub_Des.dqDes[i] = 2;
        _lcm_Pub_Des.tauDes[i] = 2;
        _lcm_Pub_Des.Kp[i] = 2;
        _lcm_Pub_Des.Kd[i] = 2;
    }
    ...
    std::cout << " _lcm_Pub_Des.qDes[i]" << _lcm_Pub_Des.qDes[1]<< std::endl;
...
void IOLcm::InputCallback(const lcm::ReceiveBuffer* rbuf, const std::string& chan,  const RealFbLcm* msg)
{
    for (int i = 0; i < 12; i++)
    {
